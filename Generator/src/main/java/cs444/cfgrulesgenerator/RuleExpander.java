...
public class RuleExpander {
    private final SortedSet<String> newNonTerminals = new TreeSet<>();
    ...
    private List<Rule> expandZeroOrOneBNFExpr(Rule rule, Range bNFExpr) {
        ...
        // first rule: N -> A C
        List<Token> firstRuleRHS = new ArrayList<>();
        for (int i = 0; i < rule.rightHandSideSize(); i++) {
            if (bNFExpr.includes(i)) {
                continue; // skip optional symbols
            }
        ...
        // second rule: N -> A B C
        List<Token> secondRuleRHS = new ArrayList<>();
        for (int i = 0; i < rule.rightHandSideSize(); i++) {
            if (bNFExpr.to == i || bNFExpr.from == i) {
                continue; // skip [ ] of BNF Expression
            }
    ...
    private List<Rule> expandZeroOrMoreBNFExpr(Rule rule, Range bNFExpr) {
        ...
        if (!newNonTerminals.contains(nonTermSymbol.toString())) {
    ...
    private void generateNewRules(Token nonTermSymbol, Rule rule, Range bNFExpr, List<Rule> ret) {
        ...
        // 3: N_B_0 -> epsilon
        List<Token> thirdRuleRHS = new ArrayList<>();
    ...
    private List<Rule> expandOrBNFExpr(Rule rule, Range bNFExpr) {
        ...
        for (int i = 0, currSubExprIndex = bNFExpr.from + 1;
            ...
            Token token;
            for (; (token = rule.getRightHandSideToken(currSubExprIndex)).type != Token.Type.PIPE &&
                    currSubExprIndex < bNFExpr.to;
                 currSubExprIndex++) {
    ...
    private int countNumOfPipes(Rule rule, Range bNFExpr) {
        int counter = 0;
        for (int i = bNFExpr.from + 1; i < bNFExpr.to; i++) {
            if (rule.getRightHandSideToken(i).type == Token.Type.PIPE) {
                counter++;
            }
        }
        return counter;
    ...
    private void addSymbolsBeforeExpr(Rule rule, Range bNFExpr, List<Token> ruleRHS) {
        for (int i = 0; i < bNFExpr.from; i++) {
            ruleRHS.add(rule.getRightHandSideToken(i));
        }
    }

    private void addSymbolsAfterExpr(Rule rule, Range bNFExpr, List<Token> ruleRHS) {
