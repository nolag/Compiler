...
public class FieldLinker extends EmptyVisitor {
    ...
    public FieldLinker(String enclosingClassName){
    ...
    public void open(MethodInvokeSymbol invoke) throws CompilerException {
        ...
        if(invoke.lookupFirst != null){
            ...
            if(dcl == null){
                List<DclSymbol> dcls = resolver.findDclOn(invoke.lookupFirst, isStatic, on);
    ...
    public void close(MethodInvokeSymbol invoke) throws UndeclaredException {
        ...
        for(Typeable mod : currentSymbols){
            ...
            if(type.isClass) throw new UndeclaredException("Class arguments", "CLASS");
    ...
    private void simpleVistorHelper(TypeableTerminal tt, String visitorType) throws UndeclaredException{
        ...
        if (resolver == null) {
            throw new UndeclaredException(visitorType, enclosingClassName);
        }
    ...
    public void visit(NegOpExprSymbol op) throws IllegalCastAssignmentException, UndeclaredException {
        ...
        if(intType.getCastablility(was.getTypeDclNode()) == Castable.NOT_CASTABLE){
            ...
            throw new IllegalCastAssignmentException(enclosingClassName, enclosingClassName, name1, enclosingClassName);
        }
    }
    ...
    public void visit(InstanceOfExprSymbol op) throws IllegalInstanceOfException, UndeclaredException {
        ...
        String where = "CLASS";
        if(lhs.isClass)
            throw new IllegalInstanceOfException(enclosingClassName, where, "Classes:", "CLASS");

        if(!rhs.isClass)
            throw new IllegalInstanceOfException(enclosingClassName, where, "non class:", "CLASS");

        if(!lhs.isArray && JoosNonTerminal.notAllowedForInstanceOfLHS.contains(lhs.value))
            throw new IllegalInstanceOfException(enclosingClassName, where, lhs.value, "CLASS");

        if(!rhs.isArray && JoosNonTerminal.notAllowedForInstanceOfRHS.contains(rhs.value))
            throw new IllegalInstanceOfException(enclosingClassName, where, rhs.value, "CLASS");
        currentTypes.peek().add(TypeSymbol.getPrimative(JoosNonTerminal.BOOLEAN));
    }
    ...
    public void close(CreationExpression create) throws UndeclaredException{
        ...
        for(Typeable mod : currentSymbols){
            ...
            //If it's a class this would be like the argument is String.  There is no local String.
            if(type.isClass)
                throw new UndeclaredException("Class arguments", "CLASS");
    ...
    public void visit(ArrayAccessExprSymbol array) throws CompilerException{
        ...
        if(in.isClass)
            throw new UnsupportedException("Array access for classes in " + "CLASS" + " " + enclosingClassName);

        if(!isNumeric(value, false))
            throw new UnsupportedException("Array access with non numeric " + value.getType().value + " " + "CLASS" + " " + enclosingClassName);
    ...
    private void assertIsBoolean(TypeSymbol type) throws UndeclaredException,
      BadOperandsTypeException {
        ...
        if(type.isArray || type.isClass){
            String where = "WHERE";
            throw new BadOperandsTypeException(enclosingClassName, where, ArrayPkgClassResolver.getArrayName(type.value), "CLASS");
        }

        if(!type.value.equals(JoosNonTerminal.BOOLEAN)){
            String where = "WHERE";
            throw new BadOperandsTypeException(enclosingClassName, where, type.value, "CLASS");
        }
    }
    ...
    private boolean isCastable(TypeSymbol type, String to, boolean die)
        ...
        if(type.isArray || type.isClass){
            String where = "WHERE";
            if (die) throw new BadOperandsTypeException(enclosingClassName, where, "CLASS", "CLASS");
            return false;
        }

        APkgClassResolver typeResolver = PkgClassInfo.instance.getSymbol(to);
        if (typeResolver == null) {
            throw new UndeclaredException(to, "CLASS");
        }
        if(typeResolver.getCastablility(type.getTypeDclNode()) == Castable.NOT_CASTABLE){
            String where = "WHERE";
            if (die) throw new BadOperandsTypeException(enclosingClassName, where, type.value, "CLASS");
            return false;
        }
        return true;
    }
