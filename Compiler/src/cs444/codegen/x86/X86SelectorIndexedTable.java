...
public class X86SelectorIndexedTable extends SelectorIndexedTable<X86Instruction, Size> {

    public X86SelectorIndexedTable(final X86SizeHelper sizeHelper){
        super(new X86IndexedTableData(new SelectorCellGen(), sizeHelper.getIntSize(sizeHelper.getDefaultSize())), sizeHelper);
    }

    public static class SelectorCellGen implements ISubtypeCellGen<X86Instruction, Size> {

        @Override
        public void genEmptyCelCode(final String colHeaderLabel, final String rowName,
                final InstructionHolder<X86Instruction> instructions, final SizeHelper<X86Instruction, Size> sizeHelper) {
