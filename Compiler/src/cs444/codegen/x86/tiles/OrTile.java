...
public class OrTile implements ITile<X86Instruction, Size, OrExprSymbol>{
    public static void init(){
        new OrTile();
    }

    private OrTile(){
        TileSet.<X86Instruction, Size>getOrMake(X86Instruction.class).ors.add(this);
    }

    @Override
    public boolean fits(final OrExprSymbol symbol, final Platform<X86Instruction, Size> platform) {
        return true;
    }

    @Override
    public InstructionsAndTiming<X86Instruction> generate(final OrExprSymbol op, final Platform<X86Instruction, Size> platform){
