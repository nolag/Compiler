...
public class ListedSymbolFactory extends ASTSymbolFactory {
    ...
    protected ISymbol convert(ISymbol from) {
        ...
        if (nonTerm.children.size() == 2) {
            if (nonTerm.children.get(0).getName().equals(nonTerm.getName())) {
                return new ListedSymbol(nonTerm, 0);
            } else if (nonTerm.children.get(1).getName().equals(nonTerm.getName())) {
                return new ListedSymbol(nonTerm, 1);
            }
        }
