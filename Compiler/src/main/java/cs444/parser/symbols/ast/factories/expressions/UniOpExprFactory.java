...
public class UniOpExprFactory extends ASTSymbolFactory {
    ...
    protected ISymbol convert(ISymbol from) throws OutOfRangeException, UnsupportedException, IllegalModifierException {
        if (!JoosNonTerminal.unaryExpressions.contains(from.getName().toUpperCase())) {
            return from;
        }
        ...
        for (int i = children.size() - 2; i >= 0; i--) {
            ...
            if (typeName.equals(Token.Type.EXCLAMATION.toString())) {
                child = new NotOpExprSymbol(child);
            } else if (typeName.equals(Token.Type.MINUS.toString())) {
                child = new NegOpExprSymbol(child);
            } else {
